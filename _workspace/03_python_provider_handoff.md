...
Commits: `537c0d2 feat: verify music through external providers`; `73e077d fix: align verified track provider contract`; `54d8a98 fix: harden provider retries and async caches`; `8507692 fix: bound retries and shield shared cache work`

Tests: provider suite `29 passed in 0.44s`; Task 1-3 Python regression `44 passed in 0.67s`; full Python suite `48 passed in 0.41s`.
...
Review v2 guarantees: external `Retry-After` is finite and capped at 5 seconds; cancelling one coalesced waiter does not cancel the shared computation or a surviving waiter.
