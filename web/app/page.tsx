    ...
    try {
      ...
      while (true) {
        ...
        for (const row of rows) {
          ...
          if (event.type === "playlist") { setPlaylist(event.data); setProgress(100); }
          if (event.type === "error") setError(event.message);
          if (event.type === "done") setStatus(event.ok === false ? "추천을 완료하지 못했어요." : "추천이 완성됐어요.");
        }
      }
